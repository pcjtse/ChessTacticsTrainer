...
class UpdateXiangqiStreakUseCase(private val repository: XiangqiPuzzleRepository) {

    suspend operator fun invoke(solved: Boolean) {
        val current = repository.observeUserProgress().first()
        val today   = LocalDate.now()
        val last    = current.lastPuzzleDate

        val updated: UserProgress = when {
            last == today -> current.copy(
                totalAttempted = current.totalAttempted + 1,
                totalSolved    = if (solved) current.totalSolved + 1 else current.totalSolved
            )
            last == today.minusDays(1) && solved -> current.copy(
