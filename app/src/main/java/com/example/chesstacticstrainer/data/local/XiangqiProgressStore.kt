...
private val Context.xiangqiProgressDataStore by preferencesDataStore(name = "xiangqi_progress")

class XiangqiProgressStore(private val context: Context) {

    private val RATING         = intPreferencesKey("xq_rating")
    private val STREAK         = intPreferencesKey("xq_current_streak")
    private val LONGEST        = intPreferencesKey("xq_longest_streak")
    private val LAST_DATE      = longPreferencesKey("xq_last_puzzle_epoch_day")
    private val TOTAL_SOLVED   = intPreferencesKey("xq_total_solved")
    private val TOTAL_ATTEMPTED = intPreferencesKey("xq_total_attempted")

    fun observe(): Flow<UserProgress> = context.xiangqiProgressDataStore.data.map { prefs ->
    ...
    suspend fun update(progress: UserProgress) {
        context.xiangqiProgressDataStore.edit { prefs ->
